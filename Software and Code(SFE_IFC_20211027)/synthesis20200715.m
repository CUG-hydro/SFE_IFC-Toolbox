...
%     pre=alldata_selected423{sta,6}(:,1:4);
%     years_count=tabulate(pre(:,1));
%     years_num=years_count(years_count(:,2)>=365,1);
%     for jj=1:size(years_num,1)
%         annual_pre(jj,1)=sum(pre(pre(:,1)==years_num(jj),4));
%     end
%     mean_pre(sta,1)=mean(annual_pre);
%     annual_pre=[];
    %% Select peaks serise
...
%     peaks_date=datevec(peaks_datenum(:,5));
%     peaks_year=peaks_date(ismember(peaks_date(:,1),years_num),1);
%     peaksnum_count=tabulate(peaks_year);
%     peaksnum_count(peaksnum_count(:,2)==0,:)=[];
...
%                tempdate=datevec(peaks_datenum(:,5));
%                     date_peaks=[tempdate(:,1:3),peaks_datenum(:,2)];
%                     output_para2 = validatation_poisson_ks(date_peaks,flow_discharge);
%                     selected_thr(sta,5:10)=output_para2(:);
    p_value=[];
    ad_value=[];
    ratio=[];
